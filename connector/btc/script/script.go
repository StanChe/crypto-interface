	...
	for i := 0; i < len(script); {
		...
		case op.Length > 1:
			if len(script[i:]) < op.Length {
				...
				return retScript, fmt.Errorf("ParseScript.Length1: %s", err.Error())
		...
		case op.Length < 0:
			...
			if len(script[off:]) < -op.Length {
				...
				return retScript, fmt.Errorf("ParseScript.Length0: %s", err.Error())
			}
			...
			default:
				err := fmt.Errorf("invalid opcode length %d",
					op.Length)
				// log.Errorf("ParseScript.Length-: %s", err.Error())
			...
			if int(l) > len(script[off:]) || int(l) < 0 {
				...
				// log.Errorf("ParseScript.BufOverflow: %s", err.Error())
				return retScript, err
			}
...
func RedeemScriptFromTxin(txin *wire.TxIn) (redeemScript []byte, err error) {
	...
	if err != nil {
		// log.Errorf("RedeemScriptFromTxin.ParseScript: %s", err.Error())
		return
	}
	if sigScript == nil {
...
func xpubFromElectrumEncoded(xpubData []byte) (xkey *hdkeychain.ExtendedKey, path []uint32, err error) {
	...
	if err != nil {
		// log.Errorf("XpubFromElectrumEncoded.NewKeyFromString: %s", err.Error())
		return
	}
	if xkey == nil {
	...
	/*
	   for _, index := range path {
	       ...
	       if err != nil {
	           // log.Errorf("XpubFromElectrumEncoded.Child: %s", err.Error())
	       }
	       if xkey == nil {
...
func ChildFromXkeyPath(xkey *hdkeychain.ExtendedKey, path []uint32) (*hdkeychain.ExtendedKey, error) {
	...
	for _, index := range path {
		...
		if err != nil {
			// log.Errorf("ChildFromXpubPath.Child: %s", err.Error())
			return nil, err
		}
		if xkey == nil {
			err = fmt.Errorf("empty child key")
			// log.Errorf("ChildFromXpubPath.Child(%d): %s", index, err.Error())
			return nil, err
		}
	}
	return xkey, nil
...
func PubkeysIndexPathFromScript(redeem []byte, xpub *hdkeychain.ExtendedKey) (
	...
	if err != nil {
		// log.Errorf("PubkeysFromScript.ParseMsScript: %s", err.Error())
		return
	}
	if msScript == nil {
	...
	if sz < sizeMultisigScriptMin ||
		...
		// log.Errorf("PubkeysFromScript: %s", err.Error())
		return
	}
	var n byte
	...
	if xpub != nil {
		...
		if err != nil {
			// log.Errorf("PubkeysFromScript.ECPubKey: %s", err.Error())
			return
		}
	}

	var xpubi *hdkeychain.ExtendedKey
	var path []uint32
	...
	for i := ofs; i < sz-2; i++ {
		...
		xpubi, path, err = xpubFromElectrumEncoded(msScript[i].Data[1:])
		if err != nil {
			// log.Errorf("PubkeysFromScript.XpubFromElectrumEncoded[%d]: %s", i-ofs, err.Error())
			return
		}
		...
		if pubSample != nil {
			pub, err = xpubi.ECPubKey()
			if err != nil {
				// log.Errorf("PubkeysFromScript.ECPubKey[%d]: %s", i-ofs, err.Error())
				return
			}
		...
		xpubi, err = ChildFromXkeyPath(xpubi, path)
		if err != nil {
			// log.Errorf("PubkeysFromScript.ChildFromXpubPath[%d]: %s", i-ofs, err.Error())
			return
		}
		...
		if pub == nil {
			...
			// log.Errorf("PubkeysFromRedeemScript.ECPubKey: %s", err.Error())
			return
		}
		pubkeys[i-ofs] = pub
	}
...
func MultisigScriptFromPubkeys(m byte, pubkeys []*btcec.PublicKey, index int) (script []byte, indexSorted int, err error) {
	...
	if n <= 0 {
		...
		// log.Errorf("MultisigScriptFromPubkeys: %s", err.Error())
		return
	}
	...
	for i := range pubkeys {
		if bytes.Compare(pubkeysCompressed[index], sortedPubkeysCompressed[i]) == 0 {
			indexSorted = i
			break
		}
	}
